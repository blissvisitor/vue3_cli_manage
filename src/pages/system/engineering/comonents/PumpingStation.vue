...
export default {
	...
	computed: mapGetters({
		...
		monitorPoints: 'system_store/GET_MONITORPOINTS',
	...
	methods: {
		...
		_initPartition() {
			...
			const { cwsCd, wfctCd } = that.data;
			// console.warn(that.prtypeList);
			...
			that.formList.forEach((item = {}) => {
				...
				if (item.name === 'wfctCd') {
					const children =
						Array.isArray(that.monitorPoints) && that.monitorPoints[0]
							? that.monitorPoints.map((it = {}) => {
									return {
										...it,
										label: it.label
									};
					...
					wfctCd ? (that.ruleForm[item.name] = wfctCd) : (that.ruleForm[item.name] = value);
					item.children = children;
				}
			});
		},
