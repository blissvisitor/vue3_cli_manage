...
function appendRequiredResources(copyDirectories, source, keyField) {
    if (/(^dse-)|(^gisframework-)/.test(keyField)) {
        ...
        copyDirectories.push({
            ...
            to: '.' + source + keyField,
            ignore: ['*.md', 'package.json', 'package-lock.json', '*.map', '*.txt']
        });
    } else if (/^lodash/.test(keyField)) {
        //
        copyDirectories.push({
            from: '.' + source + keyField + '/lodash.min.js',
            to: '.' + source + keyField + '/lodash.min.js',
            toType: 'file'
        });
    } else {
...
/**
 *  @description 复制项目中使用非 ES6模块. 如 AMD或者JQuery.
